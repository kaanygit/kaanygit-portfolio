...
const BlogPage:React.FC=()=>{
    ...
    return(
        <Transition show={showPageTransition} enter='transition-opacity duration-1000' enterFrom='opacity-0' enterTo='opacity-100'>
            <section className="mx-auto flex min-h-full w-full flex-col  px-24 pt-20 pb-16 transform duration-300 ease-in-out">
                <div className="flex justify-center items-center w-full text-center text-background-color-2 text-5xl font-bold mb-10"><span>BLOG</span></div>
                <div className="flex items-center grid 2xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-1 sm:grid-cols-1 gap-6  ">
                
                    {Array.from({length:4}).map((_,index:number)=>(
                    <Card key={index} shadow={false} className="relative grid h-[20rem] w-[20rem] max-w-[28rem] items-center justify-center overflow-hidden text-center">
